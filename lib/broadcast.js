 ...
 * @returns {Promise<{ok: boolean, connectionLost?: boolean, error?: string}>}
 */
async function sendOne(sock, jid, payload, maxRetries = DEFAULT_MAX_RETRIES) {
  let lastErr;
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      await sock.sendMessage(jid, payload, { quoted: null });
      return { ok: true };
    } catch (e) {
      lastErr = e;
