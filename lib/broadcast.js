...
/**
 * True when the WhatsApp WebSocket is really NOT open.
 * Baileys throws "Connection Closed" for momentary hiccups too — the loop
 * must ONLY stop when the socket is genuinely dead, otherwise a transient
 * blip after the first send would cancel the rest of the broadcast.
 * Unknown/absent ws (mocks, legacy) is treated as open so errors stay
...
function socketIsOpen(sock) {
  try {
    ...
    if (typeof ws.readyState === 'number') return ws.readyState === 1; // WebSocket.OPEN
    return true;
  } catch {
    return true;
  }
...
/**
 * Wait for the server's verdict on a sent message:
 *   'sent'  — server accepted it (status >= SERVER_ACK)
 *   'error' — server REJECTED it (status ERROR — invalid/restricted recipient)
 *   null    — no verdict within timeout (sendMessage resolved → treated as
 *             accepted, delivery unverified)
 */
 ...
 *
 * Failure isolation: a normal error (or a server rejection ack) fails ONLY
 * this target — the caller continues with the next one. The loop is aborted
 * (connectionLost) only when the WebSocket itself is actually closed.
 *
...
async function sendOne(sock, jid, payload, maxRetries = DEFAULT_MAX_RETRIES, opts = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Yalnız socket həqiqətən qapalıdırsa loop dayanır (real connection loss).
      const isConnectionLost = CONNECTION_ERROR_HINTS.some((h) => msg.includes(h)) && !socketIsOpen(sock);
      if (isConnectionLost) {
